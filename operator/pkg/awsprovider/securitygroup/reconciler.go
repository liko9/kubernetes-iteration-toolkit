...
func (p *Provider) For(ctx context.Context, clusterName string) ([]*string, error) {
	...
	if err != nil {
		return []*string{}, fmt.Errorf("getting control plane instances for %v, %w", clusterName, err)
	}
...
func (p *Provider) getSecurityGroupFor(ctx context.Context, instanceID string) ([]*string, error) {
	...
	if err != nil {
		return []*string{}, fmt.Errorf("describing ec2 instance id %s, %w", instanceID, err)
	}
	if len(output.Reservations) != 1 {
		return []*string{}, fmt.Errorf("unknown reservations count for %s", instanceID)
	}
	if len(output.Reservations[0].Instances) != 1 {
		return []*string{}, fmt.Errorf("missing desired instance id %s", instanceID)
	}
	securityGroups := make([]*string, 0, len(output.Reservations[0].Instances[0].SecurityGroups))
